...
bool ClientSession::PostAccept()
{
    ...
    SOCKET* listenSocket = GIocpManager->GetListenSocket();

    if (FALSE == UserAcceptEx(*listenSocket, mSocket, buffer, 0, sizeof(SOCKADDR_IN) + 16, sizeof(SOCKADDR_IN) + 16, &dwBytes, (LPOVERLAPPED)acceptContext))
    {
...
void ClientSession::DisconnectRequest(DisconnectReason dr)
{
    ...
    if (FALSE == DisconnectEx(mSocket, (LPOVERLAPPED)context, 0, 0))
    {
        if (WSA_IO_PENDING != WSAGetLastError())
        {
            printf("DiscconectEx Error!\n");
